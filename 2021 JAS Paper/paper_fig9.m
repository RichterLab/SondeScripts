...
load virtual_sondes_10m_bin_radius
meanufit = squeeze(meanufit);
meanzfit = squeeze(meanzfit);
RMWmed = 0.5.*(RMWmin+RMWmax);
radRMW = RMWmed./12;
meanufit(7,45) = 0.5*(meanufit(7,44) + meanufit(7,46));
...
hold all
cmapct = 1;
for ir = 1:10%length(RMWmin)
    subplot('position',[0.07 0.61 0.89 0.36])
    hold on
    plot(meanufit(ir,:),meanzfit(ir,:),'-s','color',[cmap(cmapct,1) cmap(cmapct,2) cmap(cmapct,3)],'markerfacecolor',[cmap(cmapct,1) cmap(cmapct,2) cmap(cmapct,3)])
    
    Ucoeffs = polyfit(log(meanzfit(ir,3:6)),meanufit(ir,3:6),1) % fit from 23 to 100 m
    uline = polyval(Ucoeffs,log(meanzfit(ir,3:6)));
    %     plot(uline,meanzfit(ir,3:6),'k')
    ...
    radius{cmapct} = [num2str(RMWmin(ir)/12,'%.2f') ' < R/RMW \leq ' num2str(RMWmax(ir)/12,'%.2f')];
...
plot(radRMW(1:10),CD,'-','color',[0.5 0.5 0.5])
...
plot(radRMW(1:10),ustar,'-','color',[0.5 0.5 0.5])


hold all
cmapct = 1;
for ir = 1:10%0:find(x<30000,1,'last')
    subplot('position',[0.07 0.61 0.89 0.36])
    hold on
    Ucoeffs = polyfit(log(meanzfit(ir,3:6)),meanufit(ir,3:6),1) % fit from 23 to 100 m
    uline = polyval(Ucoeffs,log(meanzfit(ir,3:6)));
    plot(uline,meanzfit(ir,3:6),'k','linewidth',2,'HandleVisibility','off')
...
% save('fig10extra.mat','CD','radRMW','u10','ustar')